    ...
    <td class="px-6 py-4 flex justify-center items-center space-x-reverse space-x-1">
        <div x-data="{show:false}">
            <div class="cursor-pointer relative">
                <svg class="icon w-6 h-6 hover:stroke-green-600" @click="generateQrCode('{{url($link->slug)}}','qr-code-{{$link->id}}');show = !show">
                    <use xlink:href="#scan"/>
                </svg>
                <div x-show="show"
                    ...
                    <a id="qr-download-{{$link->id}}" @click="downloadQrCode('{{$link->id}}')" download="{{ $link->slug }}"
